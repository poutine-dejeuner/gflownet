...
class Photo(GFlowNetEnv):
    def __init__(
        ...
        **kwargs,
        ...
        self.pad_token = torch.zeros(action_dim)
        self.max_length = max_length
        self.eos_idx = -1
        self.pad_idx = 0
        # Dictionaries
        self.source = torch.zeros(action_dim)
    ...
    def get_action_space(self):
        ...
        The actions are tuples of length action_dim + 1, where the value at position d
        ...
        - EOS: (inf, inf, ..., inf, inf)
        """
        actions_dim = self.action_dim + 1
        self.eos = tuple([np.inf] * actions_dim)
        self.representative_no_source = tuple([0.0] * self.action_dim) + (0,)
        self.representative_source = tuple([0.0] * self.action_dim) + (1,)
    ...
    def states2tensor(
        ...
        """
        in:
            states: set(Tensor(batch, action_dim))

        out:
            tensor_states: Tensor(batch, max_length, action_dim)
        """
        states = list(states)

        if states[0].ndim == 1:
            batch = 1
            states = [state.unsqueeze(dim=0) for state in states]
        else:
            batch = states[0].shape[0]

        states = torch.stack(states, dim=1)
        tensor_states = torch.zeros((batch, self.max_length, self.action_dim))
        tensor_states[:, :states.shape[1], :] = states 
